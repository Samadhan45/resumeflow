...
const resumeReducer = (state: Resume, action: Action): Resume => {
  switch (action.type) {
    ...
    case 'APPLY_AI_SUGGESTION': {
      const { section, key, index, value, id } = action.payload;
      // Remove the suggestion
      const newState = { ...state, aiSuggestions: state.aiSuggestions.filter(s => s.id !== id) };

      // Apply the change based on section
      // This is dynamic based on assumption of simple structured updates.
      // For complex updates, we trust the mapping in the component calling this.
      if (section === 'summary') {
        return { ...newState, summary: value };
      } else if (section === 'experience' && index !== undefined && key) {
        return {
          ...newState,
          experience: newState.experience.map((exp, i) => i === index ? { ...exp, [key]: value } : exp)
        };
      } else if (section === 'projects' && index !== undefined && key) {
        return {
          ...newState,
          projects: newState.projects.map((proj, i) => i === index ? { ...proj, [key]: value } : proj)
        };
      } else if (section === 'personalInfo' && key) {
        // @ts-ignore
        return { ...newState, personalInfo: { ...newState.personalInfo, [key]: value } };
      } else if (section === 'general' && key) {
