...
export const ModernTemplate = ({ resume }: TemplateProps) => {
    ...
    return (
        <div className="p-8 md:p-12 space-y-4 flex-1 overflow-auto resume-preview text-black h-full" style={themeStyle}>
            {/* Header */}
                        ...
                        {(proj.techStack || []).length > 0 && (
                            <div className="flex flex-wrap gap-1 pt-1">
                                ...
                                {(proj.techStack || []).map((tech, i) => (
                                    <Badge key={i} variant="secondary" className="px-1.5 py-0.5 text-xs font-normal border border-gray-200">{tech}</Badge>
                                ))}
                            </div>
                        )}
                    </div>
